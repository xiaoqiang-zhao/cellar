...
`article-list`，将数据循环一下展示到页面中，数据在`articles/articles.json`(此文件由工程化辅助工具 `publish` 生成) 中，前端直接请求此文件来获取文章列表数据。还有一个技术问题没搞定：vue-router 的 `v-link` 没得到支持，直接拼字符串实现了。还有发布时间也没做显示，欢迎贡献代码。
